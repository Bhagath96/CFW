...
import subscriptionTypeValidation from '../validations';
import ImgaeUpload from '../../../common/components/custom/ImageUpload';
...
function CreateSubscriptionType(props) {
    const { id } = useParams();
    ...
    useEffect(() => {
        if (id) {
            dispatch(Actions.fetchSubscriptionTypeById(id));
        }
    }, [id]);
    ...
    const submit = (values) => {
        ...
        if (id) {
            if (values.labels?.length > 0) {
                ...
                dispatch(Actions.updatesSubscriptionTypeData(id, stateObjectToSend));
            } else {
            ...
            if (values.labels?.length > 0) {
                stateObjectToSend.labels = values.labels;
                dispatch(Actions.saveSubscriptionTypeData(stateObjectToSend));
            } else {
                SetLabelError(I18n.t('At_least_one_label_must_be_entered'));
            }
    ...
    const accept = [MIME_TYPES.PNG, MIME_TYPES.JPEG, MIME_TYPES.JPG];
    let acceptedFiles = '';
            ...
            <Form onSubmit={handleSubmit(submit)} autoComplete='off'>
                <Grid container>
                    ...
                    <Grid item xs={12} className={classes.item}>
                        <Field name='icon' label={I18n.t('icon')} type='text' placeholder={I18n.t('supported_formats_and_size', { types: acceptedFiles, maxSize: 5 })} component={renderTextAreaField} />
                        <ImgaeUpload icon={true} withoutMime={true} onImageUpload={(val) => {
                            change('icon', val);
                        }} />
                    </Grid>
...
export default
    connect(state => ({
        initialValues: state[STATE_REDUCER_KEY].fetchSubscriptionTypeById.data
    }))(reduxForm({
        form: 'CreateSubscriptionType',
        enableReinitialize: true,
        validate: subscriptionTypeValidation
