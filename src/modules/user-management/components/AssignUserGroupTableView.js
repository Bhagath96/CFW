...
const AssignUserGroupUnderUserView = (props) => {
    ...
    return (
        <Form onSubmit={props.handleSubmit(submit)}>
            <Grid container>
                <Grid item xs={12} sm={6} ><Field component="input" name='member' type="checkbox" onClick={onMemberClick} /><span>{I18n.t('user_group_belongs_to_the_current_user')}</span><br /><br /></Grid>
                {!member && <Grid item xs={12} sm={6}><Field component="input" name='unassignedUsersOnly' type="checkbox" onClick={onUnassignedUsersOnlyClick} /><span>{I18n.t('user_group_dont_have_any_user_associations')}</span><br /><br /></Grid>}
            </Grid>
            <MUIDataTable
