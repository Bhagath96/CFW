...
const RoleAssignToUserTableView = (props) => {
    ...
    const { assignRoleToUser: { data: assignUserGroupData = [], requestInProgress = false, searchKeys = [] } = {} } = useSelector(state => _.get(state, STATE_REDUCER_KEY, {}));
    ...
    return (
        <Form onSubmit={props.handleSubmit(submit)}>
            <Grid container>
                <Grid item xs={12} sm={6} ><Field component="input" name='member' type="checkbox" onClick={onMemberClick} /><span>{I18n.t('users_belongs_to_the_current_role')}</span><br /><br /></Grid>
                {!member && <Grid item xs={12} sm={6}><Field component="input" name='unassignedUsersOnly' type="checkbox" onClick={onUnassignedUsersOnlyClick} /><span>{I18n.t('users_dont_have_any_role_association')}</span><br /><br /></Grid>}
            </Grid>
            <MUIDataTable
...
const mapDispatchToProps = dispatch => ({
    ...
    setSelectedUsers: (data) => dispatch(setSelectedIds({ key: TABLE_IDS.LIST_ASSIGN_ROLE_TO_USER, selectedIds: data }))
});
